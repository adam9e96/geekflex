...
const EditReview = () => {
  ...
  if (contentLoading || isLoadingReview) {
    return <LoadingSpinner message="정보를 불러오는 중..." className={styles.loading} />;
  }

  // 에러 발생
  if (contentError || reviewError) {
    return <ErrorMessage message={contentError || reviewError} className={styles.error} />;
  }

  // 콘텐츠 정보 없음
  if (!content) {
    return <ErrorMessage message="콘텐츠 정보를 찾을 수 없습니다." className={styles.error} />;
  }
      ...
      <div className={styles.innerContainer}>
        ...
        <div className={styles.movieCard}>
          ...
          <div className={styles.movieInfo}>
            <h1 className={styles.movieTitle}>{content.title}</h1>
            {content.original_title && content.original_title !== content.title && (
              <p className={styles.movieOriginalTitle}>{content.original_title}</p>
            )}
            <div className={styles.movieRating}>
              <i className="fas fa-star"></i>
              <span>{(content.vote_average || content.voteAverage || 0).toFixed(1)} TMDB</span>
            </div>
            <p className={styles.movieMeta}>{formatMeta()}</p>
          </div>
        </div>
        ...
        <div className={styles.formContainer}>
          <div className={styles.formHeader}>
            <h2 className={styles.formTitle}>상세 리뷰 수정</h2>
            <p className={styles.formSubtitle}>리뷰 내용을 수정해주세요</p>
          </div>
          ...
          <form className={styles.form} onSubmit={handleSubmit}>
            ...
            <div className={styles.section}>
              <label className={styles.label}>
                감상한 날짜 <span className={styles.required}>*</span>
              </label>
              <div className={styles.dateInputWrapper}>
                <input
                  ...
                  required
                />
                <i className={`fas fa-calendar-alt ${styles.dateIcon}`}></i>
              </div>
            </div>
            ...
            <div className={styles.section}>
              <label className={styles.label}>
                강추 포인트 <span className={styles.optional}>(선택)</span>
              </label>
            ...
            <div className={styles.section}>
              <label className={styles.label}>
                엔딩 크레딧 <span className={styles.optional}>(선택)</span>
              </label>
              <div className={styles.toggleWrapper}>
                <span className={styles.toggleLabel}>엔딩 크레딧이 있나요?</span>
            ...
            <div className={styles.section}>
              <label className={styles.label}>
                리뷰 제목 <span className={styles.optional}>(선택)</span>
              </label>
              <input
                type="text"
            ...
            <div className={styles.section}>
              ...
              <textarea
                ...
                required
              />
              <div className={styles.charCount}>{reviewContent.length}/5,000자</div>
            </div>
            ...
            <div className={styles.section}>
              <label className={styles.label}>
                사진 업로드 <span className={styles.optional}>(선택)</span>
              </label>
              <div className={styles.uploadArea}>
                ...
                >
                  <i className={`fas fa-arrow-up ${styles.uploadIcon}`}></i>
                  <p className={styles.uploadText}>클릭하여 사진을 업로드하세요</p>
                  <p className={styles.uploadHint}>JPG, PNG 파일 (최대 10MB)</p>
                </div>
              </div>
